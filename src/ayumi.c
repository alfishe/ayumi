...
/// Apply filter to current sample in audio stream
/// @param dc
/// @param index
/// @param x
/// @return
/// @details The function updates the state of the filter by adding the current sample
///   to the sum of the buffer minus the oldest value in the buffer, then storing the
///   current sample in the buffer. The function then subtracts the sum of the buffer
///   divided by the buffer size from the current sample, effectively removing the
///   DC offset from the signal. Finally, the filtered value is returned.
static double dc_filter(struct dc_filter *dc, int index, double x)
{
    dc->sum += -dc->delay[index] + x;
    dc->delay[index] = x;

    double result = x - dc->sum / DC_FILTER_SIZE;
