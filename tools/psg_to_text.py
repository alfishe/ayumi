...
def get_frame_data(header, file_data):
    ...
    # Check for PSG file signature
    if file_data[:4] != b'PSG\x1a':
        return False
    ...
    while index < len(file_data):
        command = ord(chr(file_data[index]))
        index += 1
        if command < 16:
            r[command] = ord(chr(file_data[index]))
            index += 1
        elif command == 0xfd:
            break
        elif command == 0xff:
...
if __name__ == '__main__':
    ...
    # Parse PSG file content
    frame_data = get_frame_data(Header_template, file_data)
    if not frame_data:
        print('Unknown format')
        sys.exit(0)

    # Save converted text into the output file
